...
def create_arguments():
    ...
    parser.add_argument('-p', '--parameters_msgf', required=True, type=str,
                        help='MS-GF+ parameter FOLDER (with parameter files) location on GCP (relative to '
                             'bucket_name_config)')
    parser.add_argument('-s', '--study_design_location', required=False, type=str,
    ...
    parser.add_argument('-x', '--pr_ratio', required=False, type=str,
                        help='Global proteomics ratio.txt results file (for inferred PTM searches)')
    return parser
...
class MSGFConfigurationGenerator:
    ...
    def sanitize_options(self):
        ...
        if self.args.study_design_location is not None:
    ...
    def load_template(self):
        ...
        try:
            ...
            raise ValueError(f'The value {self.experiment_prot} passed in for the <experiment_prot> argument is not '
                             f'supported. Only one of the following are expected: pr, ph, ub, ac, pr-lfqp, ph-lfqp')
...
def main():
    ...
    if opts.pr_ratio is not None:
        ...
        json_data['proteomics_msgfplus.pr_ratio'] = ""
