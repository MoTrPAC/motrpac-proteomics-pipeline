...
def arg_parser():
    ...
    parser.add_argument('-p', '--parameters_msgf', required=True, type=str, help='MS-GF+ parameter FOLDER (with parameter files) location on GCP (relative to bucket_name_config)')
    ...
    parser.add_argument('-r', '--results_prefix', required=True, type=str, help='Results files name prefix (which will end in _ratio.txt and _RII-peptides.txt')
    parser.add_argument('-x', '--pr_ratio', required=False, type=str, help='Global proteomics ratio.txt results file (for infered PTM searches)')
    return parser
